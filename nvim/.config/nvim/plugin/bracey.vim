...
let g:bracey_browser_command = 0                        " string command to laucnch the browser, default (0) is xdg-open
let g:bracey_auto_start_browser = 1                     " (false: 0, true: 1) whether or not to start the browser (by running g:bracey_browser_command) when bracey is started
let g:bracey_refresh_on_save = 0                        " (false: 0, true: 1) whether or not to reload the current web page whenever its corresponding buffer is written
let g:bracey_eval_on_save = 1                           " (false: 0, true: 1) whether or not to evaluate a buffer containing javascript when it is saved
let g:bracey_auto_start_server = 1                      " (false: 0, true: 1) whether or not to start the node server when :Bracey is run
let g:bracey_server_allow_remote_connections = 0        " (false: 0, true: 1) whether or not to allow other machines on the network to connect to the node server's webpage.
...
let g:bracey_server_path = 'http://127.0.0.1'           " (string) address at which the node server will reside at (should start with 'http://' and not include port)
let g:bracey_server_log = '/tmp/bracey_server_logfile'  " (string) location to log the node servers output
